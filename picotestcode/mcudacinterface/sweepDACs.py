...
# Example of initializing SPI and AMC7836
    ...
    if poll_obj.poll(0):
        ...
        if input_data == "*IDN?":
            ...
            try:
                ...
                print("setting channel",channel,"with voltage",voltage)
            ...
            try:
                ...
                channel = int(channel)
                print("reading current on channel",channel)
...
# Test code after here
    ...
    #amc.sweep_dac_sine_wave(0, 1, 31, 0.01)
    #print("temperature=",amc.read_temperature())
    #time.sleep(1)
    ...
    for channel in range(1):
        for i in range(15):
            ...
            amc.set_output_voltage(channel, voltage)
            time.sleep(0.001)
            print("DAC/ADC",channel)
            print("  setting dac to",voltage,"V")
            print("  read adc current",amc.read_output_current(channel),"A")
            time.sleep(0.001)
        
        
#amc.set_dac_voltage(0, 5)

# Amplitude = 1 (full scale), DC offset = 0 (centered), 50 steps, 0.1 second delay between points
# while True:
#    amc.sweep_dacs_sine_wave(amplitude=1, dc_offset=0, steps=50, delay=1)
